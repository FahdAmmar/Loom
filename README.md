...
- **Drag-and-drop reordering and a workspace switcher are still not
  built** — the general brief mentions them alongside things that _are_
  built now, but the phase-by-phase roadmap never actually placed them in
  Phase 6 (favorites and recents did land there, and now are built);
...
(Kanban, `@dnd-kit`); workspace export/import as JSON plus a
one-click "reset to demo content"; 5 built-in templates; drag-and-drop
reordering for pages (with re-parenting) and blocks, replacing the
original move-up/move-down-only design; real file upload for images
(data URI, since there's still no backend); TypeScript strict mode,
actually turned on this time; and a Vitest suite covering the pure
`lib/` functions, the mock API layer's CRUD/cascade-delete behavior, and
two specific store-level race conditions found and fixed along the way.
